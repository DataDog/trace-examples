...
import (
	...
	"github.com/DataDog/trace-examples/go/twirp"
...
func main() {
	...
	hooks := twirp.ChainHooks(
		statsd.NewStatsdServerHooks(LoggingStatter{ioutil.Discard}),
		twirptrace.NewServerHooks(),
	)
	server := example.NewHaberdasherServer(&randomHaberdasher{}, hooks)
	traced := twirptrace.WrapServer(server)
	log.Fatal(http.ListenAndServe(":8080", traced))
}
